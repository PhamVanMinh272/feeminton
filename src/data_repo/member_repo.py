...
class MemberRepo:
    ...
    def get_all_members(self):
        ...
        players = [{"id": row[0], "name": row[1], "group": {"id": row[2]}} for row in rows]
        return players
    ...
    def get_member_by_id(self, member_id: int):
        ...
        self._cursor.execute("SELECT members.id, nickname, group_id, gender FROM members join users on members.user_id= users.id where members.id = ?", (member_id,))
        row = self._cursor.fetchone()
        member = {"id": row[0], "nickname": row[1], "groupId": row[2], "gender": row[3]}
        ...
        unit_fee = min_fee_groups[member["groupId"]][member["gender"]]
        ...
        schedule_count = schedule_count_row[0] if schedule_count_row[0] is not None else 0
        logger.info(f"Member ID {member_id} - Schedule count for next month: {schedule_count}")

        estimated_bill = unit_fee * schedule_count - member["currentMonthRefund"]
